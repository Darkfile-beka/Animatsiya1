...
const lines = [];
let colors = [
  new THREE.Color("#c31432"),
  new THREE.Color("#CB3066"),
  new THREE.Color("#6B48FF"),
  new THREE.Color("#125D98")
];
let galaxyColors = [
  new THREE.Color("#c31432").multiplyScalar(0.7),
  new THREE.Color("#CB3066").multiplyScalar(0.7),
  new THREE.Color("#6B48FF").multiplyScalar(0.7),
  new THREE.Color("#125D98").multiplyScalar(0.7)
];
function dots() {
...
class Sparkle extends THREE.Vector3 {
  setup(origin, color) {
    ...
    this.v.x *= Math.random() > 0.7 ? 1 : -1;
    /* Y Speed */
    this.v.y = THREE.MathUtils.randFloat(0.001, 0.006);
    this.v.y *= Math.random() > 0.7 ? 1 : -1;
    /* Z Speed */
    this.v.z = THREE.MathUtils.randFloat(0.001, 0.006);
    this.v.z *= Math.random() > 0.7 ? 1 : -1;

    this.size = Math.random() * 4 + 0.7 * pixelRatio;
    this.slowDown = 0.4 + Math.random() * 0.58;
    this.color = color;
  }
  update() {
    if (this.v.x > 0.002 || this.v.y > 0.002 || this.v.z > 0.002) {
      this.add(this.v);
